...
if __name__ == '__main__':
    ...
    model = YOLOv1(device=device, input_size=config.img_size, num_classes=20, trainable=False, model_name='resnet18',
                   pretrained=config.pretrained).to(device)
    weight_path = Path(config.save_folder) / 'yolov1_voc_best.pth'
