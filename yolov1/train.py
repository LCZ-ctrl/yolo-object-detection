...
import config
from eval import predict_yolov1_batch, build_gts_from_targets, evaluate_map
...
def train():
    ...
    best_path = save_dir / "yolov1_voc_best.pth"
    ...
    # -------------------- Multi-scale --------------------
    multi_scale_sizes = getattr(config, 'multi_scale_sizes', [640])
    ...
    tqdm.write('\n🎉 Training complete!')
    tqdm.write(f"✅ Training completed in {execution_time_hours:.2f} hours")
    tqdm.write(f'💾 Best model saved with mAP@0.5: {best_map * 100:.2f}%')
    ...
    ## mAP curve
    plt.subplot(1, 2, 2)
    plt.plot(epochs, [x * 100 for x in history['mAP']], label='mAP@0.5', color='#d62728', marker='o', markersize=4)
    plt.xticks(np.arange(0, len(epochs) + 1, 10))
    plt.xlabel('Epochs')
    plt.ylabel('mAP (%)')
    ...
    ## save and show
    plt.tight_layout()
    plot_path = save_dir / 'yolov1_training_metrics.png'
