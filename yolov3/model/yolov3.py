...
class YOLOv3(nn.Module):
    def __init__(self, device, num_classes=20, anchor_size=None, trainable=False, conf_thresh=0.005, nms_thresh=0.6,
        ...
        self.obj_preds = nn.ModuleList(
            [nn.Conv2d(int(head.reg_out_dim), 1 * self.num_anchors, kernel_size=1)
             for head in self.non_shared_heads]
        )
        self.cls_preds = nn.ModuleList(
            [nn.Conv2d(int(head.cls_out_dim), self.num_classes * self.num_anchors, kernel_size=1)
             for head in self.non_shared_heads]
        )
        self.reg_preds = nn.ModuleList(
            [nn.Conv2d(int(head.reg_out_dim), 4 * self.num_anchors, kernel_size=1)
             for head in self.non_shared_heads]
        )
